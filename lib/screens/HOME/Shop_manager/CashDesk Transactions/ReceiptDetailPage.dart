...
class ReceiptDetailPage extends StatelessWidget {
  ...
  const ReceiptDetailPage({required this.receipt, Key? key}) : super(key: key);

  @override
  Widget build(BuildContext context) {
    print('Receipt details: ${receipt.items}'); // Логирование для проверки данных
    return Scaffold(
      appBar: AppBar(
           ...
           ...receipt.items.map((item) => Text(
            ...
            Center(
              child: ElevatedButton.icon(
                onPressed: () => _printPdf(receipt),
                icon: const Icon(Icons.print),
                label: const Text('Распечатать копию чека'),
  ...
  Future<void> _printPdf(ReceiptItem receipt) async {
